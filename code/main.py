...
def HistProcess(df):
    ...
    df_local = df
    Normalize(df_local, Min_Max_Normalize)
    # df_max = df.max().max()
    # df_min = df.min().min()
    # print ("df min: ", df_min, "max: ", df_max)
    # df_local = (df - df_min) / (df_max - df_min + 1e-6)
    # df_local = df
    # Normalize(df_local, Min_Max_Normalize)
    return df_local
...
def CalcHist(df, HistSize):
    ...
    with Pool(processes=6) as p:
        res = [p.apply_async(CalcHistMeta, args=(df.iloc[i], HistSize)) for i in range(df.shape[0])]
        for r in res:
            hist = r.get()
            hist_list.append(hist)
            if len(hist_list) % 10000 == 0:
                print ('Process hist rows: ', len(hist_list))
...
def gen_group_statistic_features(df, cols_list):
    ...
    for cols in cols_list:
        if cols == 'all':
            ...
            prefix = cols[0] + "_"
            df_cols = df[cols]
        statistic_features[prefix + "nz_mean"] = df_cols.apply(lambda x: x[x!=0].mean(), axis=1)
        statistic_features[prefix + "nz_max"] = df_cols.apply(lambda x: x[x!=0].max(), axis=1)
        statistic_features[prefix + "nz_min"] = df_cols.apply(lambda x: x[x!=0].min(), axis=1)
        statistic_features[prefix + "median"] = df_cols.apply(lambda x: x[x!=0].median(),axis=1)
        statistic_features[prefix + "std"] = df_cols.apply(lambda x: x[x!=0].std(),axis=1)
        statistic_features[prefix + "skew"] = df_cols.apply(lambda x: x[x!=0].skew(),axis=1)
        statistic_features[prefix + "kurt"] = df_cols.apply(lambda x: x[x!=0].kurt(),axis=1)

        statistic_features[prefix + "mean"] = df_cols.apply(lambda x: x.mean(), axis=1)
        statistic_features[prefix + "ez"] = df_cols.apply(lambda x: len(x[x==0]), axis=1)
        statistic_features[prefix + "median"] = df_cols.apply(lambda x: x.median(),axis=1)
        statistic_features[prefix + "std"] = df_cols.apply(lambda x: x.std(),axis=1)
        statistic_features[prefix + "skew"] = df_cols.apply(lambda x: x.skew(),axis=1)
        statistic_features[prefix + "kurt"] = df_cols.apply(lambda x: x.kurt(),axis=1)

    with open(path + 'statistic_features_by_cols.pickle', 'wb+') as handle:
        pickle.dump(statistic_features, handle, protocol=pickle.HIGHEST_PROTOCOL)
    return statistic_features

def gen_sort_features(df, sort_len, prefix):
    sort_features = SortData(df, sort_len)
    features_rename_dict = dict([(f, prefix + f) for f in sort_features.columns.values])
    return sort_features.rename(columns = features_rename_dict)

def gen_group_sort_features(df, cols_list, all_sort_len, group_sort_len):
    file_name = path + '/sort_features_' + str(all_sort_len) + '_' + str(group_sort_len) + '.pickle'
    if os.path.isfile(file_name):
        with open(file_name, 'rb+') as handle:
            sort_features = pickle.load(handle)
        return sort_features
    sort_features_list = []
    for cols in cols_list:
        if cols == 'all':
            sort_features = gen_sort_features(df, all_sort_len, 'all_')
        else:
            sort_features = gen_sort_features(df[cols], group_sort_len, cols[0] + '_')
        sort_features_list.append(sort_features)
    sort_features = pd.concat(sort_features_list, axis = 1, sort = False)
    with open(file_name, 'wb+') as handle:
        pickle.dump(sort_features, handle, protocol=pickle.HIGHEST_PROTOCOL)
    return sort_features

def gen_hist_features(df, hist_size, prefix):
    hist_features = norm_cal_hist(df, hist_size)
    features_rename_dict = dict([(f, prefix + f) for f in hist_features.columns.values])
    return hist_features.rename(columns = features_rename_dict)

def gen_group_hist_features(df, cols_list, all_hist_size, group_hist_size):
    file_name = path + '/hist_features_col_norm_' + str(all_hist_size) + '_' + str(group_hist_size) + '_nz.pickle'
    if os.path.isfile(file_name):
        with open(file_name, 'rb+') as handle:
            hist_features = pickle.load(handle)
            # hist_features = hist_features.loc[:, hist_features.apply(lambda x: len(x[x != 0]), axis = 0) != 0]
        return hist_features
    hist_features_list = []
    for cols in cols_list:
        if cols == 'all':
            hist_features = gen_hist_features(df, all_hist_size, 'all_')
        else:
            hist_features = gen_hist_features(df[cols], group_hist_size, cols[0] + '_')
        hist_features_list.append(hist_features)
    hist_features = pd.concat(hist_features_list, axis = 1, sort = False)
    file_name = path + '/hist_features_col_norm_' + str(all_hist_size) + '_' + str(group_hist_size) + '.pickle'
    with open(file_name, 'wb+') as handle:
        pickle.dump(hist_features, handle, protocol=pickle.HIGHEST_PROTOCOL)
    return hist_features
...
def gen_group_features(df, all_hist_size, all_sort_len, group_hist_size, group_sort_len, group_num):
    cols_list = ['all'] + find_top_nz_cols(df, LEAK_LIST, group_num)

    # statistic_features = gen_group_statistic_features(df, cols_list)
    hist_features = gen_group_hist_features(df, cols_list, all_hist_size, group_hist_size)
    sort_features = gen_group_sort_features(df, cols_list, all_sort_len, group_sort_len)

    return pd.concat([hist_features, sort_features], axis = 1, sort = False)
...
def load_data(col):
    ...
    if FLAGS.load_from_pickle:
        with open(path + 'train_test_nonormalize.pickle', 'rb') as handle:
            df, test_ID, y_train, train_row = pickle.load(handle)
        train_leak_target = pd.read_csv(path + 'train_target_leaktarget_38_1_2018_08_20_07_48_17.csv', index_col = 'ID')
        test_leak_target = pd.read_csv(path + 'test_target_leaktarget_38_1_2018_08_20_05_11_44.csv', index_col = 'ID')
        ...
        # top_cols_pred_features = df.drop(columns = origin_cols)
        ...
        all_hist_size = 1000
        all_sort_len = 1000
        group_hist_size = 1000
        group_sort_len = 40
        group_num = 8
        feature_label = '_'.join([str(all_hist_size), str(all_sort_len), str(group_hist_size), str(group_sort_len), str(group_num)])
        # # y_train = y_train[:1000]
        # group_features = gen_group_features(df, all_hist_size, all_sort_len, group_hist_size, group_sort_len, group_num).fillna(0)
        # with open(path + 'group_features_add_statistic_' + feature_label + '.pickle', 'wb+') as handle:
        #     pickle.dump(df, handle, protocol=pickle.HIGHEST_PROTOCOL)
        with open(path + 'group_features_add_statistic_100_100_100_40_8.pickle', 'rb+') as handle:
        ...
        # print ('statistic_cols: ', statistic_cols)
        # print ('hist_cols: ', hist_cols)
        # print ('sort_cols: ', sort_cols)
        df = pd.concat([df, pred_mean, pred_max, pred_nz_mean, pred_nz_min], axis = 1, sort = False).fillna(0)
