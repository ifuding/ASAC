...
def cal_loss(pred, label, leak_target):
    blend_leak_target = leak_target.copy()
    blend_leak_target[blend_leak_target == 0] = pred[blend_leak_target == 0]
    loss = np.sqrt(metrics.mean_squared_error(label, pred.values))
    blend_leak_loss = np.sqrt(metrics.mean_squared_error(label, blend_leak_target.values))

    return loss, blend_leak_loss


def cal_leak_blend_loss(leak_train, models, valide_data, valide_label):
    # print (leak_train.head)
    # leak_train = leak_train.copy()
    preds = []
    leak_train = leak_train.loc[valide_data.index]
    for model, model_type in models:
        pred = pd.Series(model_eval(model, model_type, valide_data), index = valide_data.index)
        preds.append(pred)
        loss, blend_leak_loss = cal_loss(pred, valide_label, leak_train)
        print ('model type:', model_type, 'loss: ', loss, 'leak_blend_loss: ', blend_leak_loss)
    if len(preds) == 2:
        a = 0.85
        emsemble_pred = a * preds[0] + (1 - a) * preds[1]
        loss, blend_leak_loss = cal_loss(emsemble_pred, valide_label, leak_train)
        print ('model type: Ensemble loss: ', loss, 'leak_blend_loss: ', blend_leak_loss)
    return loss, blend_leak_loss
    ...
    for train_index, test_index in kf.split(train_data):
        ...
        for model_type in model_types:
            if model_type == 'k' or model_type == 'r':
                ...
                # print (leak_train.head)
        if model_type != 'v':
            losses.append(cal_leak_blend_loss(leak_train, onefold_models, valide_part, valide_part_label))
        # if stacking:
